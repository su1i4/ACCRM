...
import { Typography, Table, Row, Col, Button, Dropdown, Card, DatePicker, Flex, Input } from "antd";
import { CalendarOutlined, ArrowUpOutlined, ArrowDownOutlined, SearchOutlined } from "@ant-design/icons";
...
export const BankShow = () => {
    ...
    const buildQueryParams = () => {
        return {
            s: searchFilters.length > 0 ? JSON.stringify({ $and: searchFilters }) : undefined,
                ...
                if (filter.date) {
                    ...
                    if ($gte) {
                        result = result.filter(op => 
                            new Date(op.date).getTime() >= new Date($gte).getTime()
                        );
                    }
                    if ($lte) {
                        result = result.filter(op => 
                            new Date(op.date).getTime() <= new Date($lte).getTime()
                        );
                    }
                }
            });
        ...
        <Show isLoading={showLoading} headerButtons={() => false} >
            ...
            <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                ...
                <Col>
                    ...
                    >
                        <Button>
                            Сортировка {sortDirection === "ASC" ? <ArrowUpOutlined /> : <ArrowDownOutlined />}
                        </Button>
                    </Dropdown>
                </Col>
                    ...
                    render={(value, record) => {
                        ...
                        return (
                            <span style={{ color: isIncome ? "green" : "red" }}>
                                {isIncome ? "+" : "-"}{value?.toLocaleString() || 0} {record.type_currency}
                            </span>
                        );
                    }}
                />
